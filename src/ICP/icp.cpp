...
void FindClosestPointForEach(PointCloud& sourceCloud, cv::Mat& destPoints, vector<float>& distances,
...
void RejectOutlierMatches(vector<Point3f>& matches1, vector<Point3f>& matches2, vector<float>& matchDistances,
                          float maxStdDev) {
...
ICP_API float __stdcall ICP(Point3f* verts1, Point3f* verts2, int nVerts1, int nVerts2, float* R, float* t,
                            int maxIter) {
	PointCloud cloud1;
	...
	cv::Mat verts2Mat(nVerts2, 3, CV_32F, reinterpret_cast<float*>(verts2));
