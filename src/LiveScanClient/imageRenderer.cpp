...
/// <returns>indicates success or failure</returns>
HRESULT ImageRenderer::Initialize(HWND hWnd, ID2D1Factory* pD2DFactory, int sourceWidth, int sourceHeight,
                                  int sourceStride) {
...
HRESULT ImageRenderer::Draw(BYTE* pImage, unsigned long cbImage, std::vector<Body>& vBodies) {
	...
	hr = m_pBitmap->CopyFromMemory(nullptr, pImage, m_sourceStride);
