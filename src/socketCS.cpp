...
std::string Socket::ReceiveBytes() {
	...
	while (true) {
		...
		const int rv = recv(s_, buf, arg, 0);
		if (rv <= 0) break;
...
SocketServer::SocketServer(int port, int connections, TypeSocket type) {
	...
	if (bind(s_, reinterpret_cast<sockaddr *>(&sa), sizeof(sockaddr_in)) == SOCKET_ERROR) {
		closesocket(s_);
		throw "INVALID_SOCKET";
	}
...
Socket* SocketServer::Accept() {
	SOCKET new_sock = accept(s_, nullptr, nullptr);
...
SocketSelect::SocketSelect(Socket const* const s1, Socket const* const s2, TypeSocket type) {
	...
	if (select(0, &fds_, static_cast<fd_set*>(nullptr), static_cast<fd_set*>(nullptr), ptval) == SOCKET_ERROR)
